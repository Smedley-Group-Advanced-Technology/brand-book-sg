...
import { FORMATS, TYPES, BUSINESSES, GROUNDS, FONTS, defaults, render, loadLogos, fontCSS, toPNG } from './posts.js';

const $ = s => document.querySelector(s);
const KEY = 'sg-social-maker';
let state = { format: 'square', type: 'headline', business: 'smedley-group', ground: 'dark', guides: false, data: {} };
try { const s = JSON.parse(localStorage.getItem(KEY) || 'null'); if (s && FORMATS[s.format] && TYPES[s.type]) state = Object.assign(state, s); } catch {}
const save = () => { try { localStorage.setItem(KEY, JSON.stringify(state)); } catch {} };
let photo = null, logos = {}, embedded = null;
const dataFor = t => Object.assign(defaults(t), state.data[t] || {});
...
const seg = (el, items, key) => {
  ...
  for (const [k, v] of Object.entries(items)) {
    ...
    b.addEventListener('click', () => { state[key] = k; el.querySelectorAll('button').forEach(x => x.setAttribute('aria-pressed', x.dataset.k === k)); if (key === 'type') buildFields(); draw(); save(); });
    el.appendChild(b);
  }
};
...
function buildFields() {
  ...
  for (const [k, label, , kind] of TYPES[state.type].fields) {
...
const opts = extra => Object.assign({ format: state.format, type: state.type, business: state.business, ground: state.ground, data: dataFor(state.type) }, extra);
function draw() {
...
const fileName = ext => `${state.business}-${state.type}-${state.format}.${ext}`;
